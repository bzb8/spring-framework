...
public interface BeanDefinition extends AttributeAccessor, BeanMetadataElement {
	 ...
	 *
	 * 角色提示表明 {@code BeanDefinition} 是某些较大配置的支持部分，通常是外部 {@link org.springframework.beans.factory.parsing.ComponentDefinition}。
	 * 当更仔细地查看特定的{@link org.springframework.beans.factory.parsing.ComponentDefinition}时，{@code SUPPORT} beans被认为足够重要，
	 * 但在查看应用程序的整体配置时则不然。
	 */
	int ROLE_SUPPORT = 1;
	 ...
	 * 角色提示表明 {@code BeanDefinition} 提供完全后台角色，与最终用户无关。
	 * 当注册完全属于 {@link org.springframework.beans.factory.parsing.ComponentDefinition} 内部工作一部分的 bean 时，会使用此提示。
	 */
	 ...
	 * 允许检索修饰的 Bean 定义（如果有）。
	 * 请注意，此方法返回直接发起方。遍历发起方链以查找用户定义的原始 BeanDefinition。
	 */
	@Nullable

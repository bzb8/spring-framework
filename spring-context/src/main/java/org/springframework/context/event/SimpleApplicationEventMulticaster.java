 ...
 *
 * 将所有事件多播到所有已注册的listeners，让listeners忽略他们不感兴趣的事件。listeners通常会对传入的事件对象执行相应的 {@code instanceof} 检查。
 *
 ...
 *
 * 默认情况下，所有listeners都在调用线程中调用。这允许恶意listeners阻止整个应用程序的危险，但增加了最小的开销。指定备用任务执行程序，让listeners在不同的线程中执行，例如从线程池中执行。
 *
 * @author Rod Johnson
